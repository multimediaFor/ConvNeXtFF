    ...
    def _resize_img(self, results):
        ...
        if self.keep_ratio:
            ...
            img, scale_factor = mmcv.imrescale(
                results['img'], results['scale'], interpolation='bicubic', return_scale=True)
...
class AddGaussianNoise(object):
    ...
    def __call__(self, results):
        if random.uniform(0, 1) < self.p:
            ...
            mean=random.uniform(0.5,1.5), 
            variance=0.5, 
            amplitude=random.uniform(0, 45)
            img = np.array(img)
            h, w, c = img.shape
...
class Pil_jpg(object):
    ...
    def __call__(self, results):
        if random.uniform(0, 1) < self.p:
            img = results['img']
            compress_val = random.randint(71, 95)
            img_cv2 = img[:,:,::-1]
